...
if __name__ == "__main__":
    ...
    for rep in range(repeats):
        ...
        stop_loop = dual_drifting_gratings_v3(win_lum, win_pol, exp_handler, p1, framerate=fr, dlp=dlp, code_on=b'1', code_off=b'Q')
        if stop_loop: break
        # gray -> gray
        reset_screen(win_lum, win_pol,
        ...
        stop_loop = dual_drifting_gratings_v3(win_lum, win_pol, exp_handler, p2, framerate=fr, dlp=dlp, code_on=b'1', code_off=b'Q')
        if stop_loop: break
        # gray -> gray
        reset_screen(win_lum, win_pol,
                     ramp_time=1, hold_time=1, framerate=fr, stim_size_lum=s1, stim_size_pol=s2,
                     start_color_lum=[0,0,0], end_color_lum=[0,0,0],
                     start_color_pol=[-1,-1,-1], end_color_pol=[0.74,0.74,0.74])
        stop_loop = dual_drifting_gratings_v3(win_lum, win_pol, exp_handler, p3, framerate=fr, dlp=dlp, code_on=b'1', code_off=b'Q')
        if stop_loop: break
        reset_screen(win_lum, win_pol,
                     ramp_time=1, hold_time=1, framerate=fr, stim_size_lum=s1, stim_size_pol=s2,
                     start_color_lum=[0,0,0], end_color_lum=[0,0,0],
