...
if __name__ == "__main__":
    ...
    p = DualDriftingGratingsParamsV2(
        mode="lum_only",
        color=1,
        contrast=1,
        SFs=[0.0085, 0.017], # cycles per pixel; 1 pixel = 0.085 degree; sf = 0.017 means 5 degrees per cycle
        TFs=[3],
    ...
    dual_drifting_gratings_v2(win_lum, win_pol, exp_handler, p, framerate=fr, dlp=dlp, code_on=b'1', code_off=b'Q')
    # gray -> black
    reset_screen(win_lum, win_pol,
                 ramp_time=4, hold_time=4, framerate=fr, stim_size_lum=s1, stim_size_pol=s2,
