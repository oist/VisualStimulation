...
import sys
sys.path.append("..")
from vstim_dual import dual_drifting_gratings_v2, DualDriftingGratingsParamsV2
...
if __name__ == "__main__":
    ...
    repeats = 8
    p1 = DualDriftingGratingsParamsV2(
        mode="lum_only",
        color=1,
        contrast=1,
    ...
    p2 = DualDriftingGratingsParamsV2(
        mode="lum_only",
        color=1,
        contrast=1,
        SFs=[0.0085], # cycles per pixel; 1 pixel = 0.085 degree; sf = 0.017 means 5 degrees per cycle
    ...
    p3 = DualDriftingGratingsParamsV2(
        mode="pol_only",
        color=0.78,
        contrast=1,
        SFs=[0.0085*1.94],
    ...
    for rep in range(repeats):
        ...
        stop_loop = dual_drifting_gratings_v2(win_lum, win_pol, exp_handler, p1, framerate=fr, dlp=dlp, code_on=b'1', code_off=b'Q')
        if stop_loop: break
        # gray -> gray
        reset_screen(win_lum, win_pol,
        ...
        stop_loop = dual_drifting_gratings_v2(win_lum, win_pol, exp_handler, p2, framerate=fr, dlp=dlp, code_on=b'1', code_off=b'Q')
        if stop_loop: break
        # gray -> gray
        reset_screen(win_lum, win_pol,
                     ramp_time=1, hold_time=1, framerate=fr, stim_size_lum=s1, stim_size_pol=s2,
                     start_color_lum=[0,0,0], end_color_lum=[0,0,0],
                     start_color_pol=[-1,-1,-1], end_color_pol=[0.74,0.74,0.74])
        stop_loop = dual_drifting_gratings_v2(win_lum, win_pol, exp_handler, p3, framerate=fr, dlp=dlp, code_on=b'1', code_off=b'Q')
        if stop_loop: break
        reset_screen(win_lum, win_pol,
                     ramp_time=1, hold_time=1, framerate=fr, stim_size_lum=s1, stim_size_pol=s2,
                     start_color_lum=[0,0,0], end_color_lum=[0,0,0],
