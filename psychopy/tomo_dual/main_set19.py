...
import sys
sys.path.append("..")
from vstim_dual import noisy_polarization_v2, NoisyPolarizationParamsV2
...
if __name__ == "__main__":
    ...
    ###### PARAMETERS BEGIN ######
    exp_name = "rec7"
    logdir = r"D:\experiments\20250121"
    com_port = "COM3" # for DLP-IO8-G
    repeats = 12
    p1 = NoisyPolarizationParamsV2(
        mode="pol_only",
        contrast_steps=[-1.000, -0.386, 0.016, 0.299, 0.535, 0.646, 0.693],
    ...
    reset_screen(win_lum, win_pol,
                 ramp_time=4, hold_time=5, framerate=fr, stim_size_lum=s1, stim_size_pol=s2,
                 start_color_lum=[-1,-1,-1], end_color_lum=[0,0,0],
                 start_color_pol=[-1,-1,-1], end_color_pol=[0.74, 0.74, 0.74])
    for rep in range(repeats):
        stop_loop = noisy_polarization_v2(win_lum, win_pol, exp_handler, p1, framerate=fr, dlp=dlp, code_on=b'1', code_off=b'Q')
        if stop_loop: break
        # gray -> gray
        reset_screen(win_lum, win_pol,
        ...
        stop_loop = noisy_polarization_v2(win_lum, win_pol, exp_handler, p2, framerate=fr, dlp=dlp, code_on=b'1', code_off=b'Q')
        if stop_loop: break
        # gray -> gray
        reset_screen(win_lum, win_pol,
                 ramp_time=1, hold_time=1, framerate=fr, stim_size_lum=s1, stim_size_pol=s2,
    ...
    # gray -> black
    reset_screen(win_lum, win_pol,
                 ramp_time=3, hold_time=5, framerate=fr, stim_size_lum=s1, stim_size_pol=s2,
