...
def locally_sparse_noise2(win, exp_handler, p: LocallySparseNoiseParams, dlp=None, code_on=b'1', code_off=b'Q', save_movie=False):
    """
    This function generates locally sparse noise (LSN) stimulus.
    ...
    for i in range(mat.shape[0]):
        ...
        image = mat[i, :, :]
        stim = visual.ImageStim(win, image=image, size=[640, 360], pos=[-320, -50])
        for j in range(stim_frames):
            frame_counter += 1
            if dlp is not None:
                if j == 0:
